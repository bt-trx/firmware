...
extern BTTRX_DISPLAY bttrx_display;
...
void BTTRX_DISPLAY::init() {
   ...
   display.drawXbm(((display_width-bt_trx_logo_width)/2), 0, bt_trx_logo_width, bt_trx_logo_height, bt_trx_logo_bits);
...
void BTTRX_DISPLAY::setStatusMessage(string message) {
   ...
   display.drawStringMaxWidth(0, (display_height-30), display_width, message.c_str());
   display.display();
}
...
void BTTRX_DISPLAY::setTransmitMessage(string message) {
   ...
   display.setFont(ArialMT_Plain_16);
   display.drawString((display_width/2), (display_height-24), message.c_str());
   display.display();
}
